  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Complete Payment & Generate Bill" maxWidth="max-w-lg">
      <div className="space-y-5">
        <div className="p-4 bg-slate-900 border border-slate-700/80 rounded-xl text-center">
          <div className="text-xs text-slate-400 font-medium uppercase tracking-wider">Grand Total Amount Due</div>
        ...
        <div>
          <label className="block text-xs text-slate-300 font-medium mb-2">Select Payment Method</label>
        ...
        {method === 'Mixed' && (
          <div className="p-4 bg-slate-900/90 border border-slate-700/80 rounded-xl space-y-3">
            <div className="text-xs font-bold text-amber-400">Mixed Payment Split Breakdown</div>
            {['Cash', 'UPI', 'Card', 'BankTransfer'].map((channel) => (
              <div key={channel} className="flex items-center justify-between gap-3 text-xs">
                <span className="text-slate-300 font-medium">{channel} ({currencySymbol})</span>
                <input
                  type="number"
                  min="0"
                  value={mixedSplits[channel]}
                  onChange={(e) => handleMixedChange(channel, e.target.value)}
                  className="w-32 px-3 py-1.5 bg-slate-800 border border-slate-700 rounded text-right text-white font-mono outline-none focus:border-indigo-500"
                />
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-end gap-3 pt-2 border-t border-slate-700/60">
          <button
            onClick={onClose}
            disabled={loading}
            className="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-xl text-xs font-semibold cursor-pointer"
          >
            Cancel
          </button>
          ...
          <button
            ...
            disabled={loading}
            className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-extrabold flex items-center gap-2 shadow-lg shadow-emerald-600/30 cursor-pointer"
          >
